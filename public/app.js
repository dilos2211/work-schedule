...
function markAsUnsaved() {
  ...
  statusMsg.innerText = "⚠ Есть несохраненные изменения";
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (dayState && dayState.shift !== 'none') {
      ...
      if (dayCalc.overtime > 0) overtimeHTML = `<div class="overtime-badge">+${dayCalc.overtime}ч надг.</div>`;
    }
...
function updateOvertimePreview() {
  ...
  overtimePreview.innerHTML = `Всего: <strong>${calc.total}ч</strong> | База: ${calc.base}ч | Надгор: <strong>${calc.overtime}ч</strong>`;
}
...
function openModal(day, holidayName = null) {
  ...
  if (!holidayBanner) {
    ...
    document.querySelector('.modal-content').insertBefore(holidayBanner, document.querySelector('.quick-shifts'));
  }

  if (holidayName) {
    holidayBanner.innerText = `Праздник: ${holidayName}`;
